...
model_type = 'resnet34'
WARM_UP_EPOCHS = 6
file_name = f'Results_ce_{noise_mode}{r}_cifar100_lr{LR}_bs{BATCH_SIZE}'
print(file_name)
remove_rate_1 = 0.975
remove_rate_2 = 0.975
remove_rate_3 = 0.975
...
with open(file_name + '.csv', "w", newline='') as file:
    ...
    writer.writerow(['round_num', 'epoch', 'acc_etrain', 'acc_test', 'FkL-Examples', 'Noise rate'])
...
class cifar_dataloader():
    ...
    def initial_data(self):
        ...
        test_dic = self.unpickle('%s/test' % root_dir)
        test_data = test_dic[b'data']
        test_data = test_data.reshape((10000, 3, 32, 32))
        test_data = test_data.transpose((0, 2, 3, 1))
        test_label = test_dic[b'fine_labels']

        train_dic = self.unpickle('%s/train' % root_dir)
...
for ite in range(I_rate):
    ...
    for epoch in range(EPOCH):
        ...
        with open(file_name + '.csv', "a", newline='') as file:
            writer = csv.writer(file)
            writer.writerow(
                [round_num, epoch, f"{acc_etrain:.2f}", f"{acc_test:.2f}", sum_FkLexample_dataset])